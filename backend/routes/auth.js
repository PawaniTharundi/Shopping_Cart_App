...
router.post('/passkey/login/begin', async (req, res) => {
  ...
  if (!user || user.authProvider !== 'passkey') return res.status(404).json({ error: 'User not found' });
...
router.get('/me', async (req, res) => {
  ...
  try {
    ...
    const user = await User.findById(decoded.userId).select('-credentialID -publicKey -counter');
    res.json({ user });
  } catch (err) {
    res.json({ user: null });
  }
